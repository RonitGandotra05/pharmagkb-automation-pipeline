...
class TestSampleProcessor:
    ...
    def setup_logging(self):
        log_dir = os.path.join('test_output', 'logs')
        os.makedirs(log_dir, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        log_file = os.path.join(log_dir, f'test_processing_{timestamp}.log')
    ...
    def setup_output_dirs(self):
        ...
        self.output_dir = os.path.join('test_output', 'processed_files')
    ...
    def process_single_sample(self):
        ...
        sample_id = "N1DL5740"
        logging.info(f"Processing test sample: {sample_id}")
        
        try:
            # Get the scraped content file
            scraped_file = os.path.join('test_output', 'scraped_content', f'{sample_id}_processed_content_20250113_001322.txt')
            if not os.path.exists(scraped_file):
                logging.error(f"Scraped content file not found: {scraped_file}")
                return
                
            # Get the input Excel file from step1 output
            input_excel = os.path.join('step1_phenotype_genotype_output', 'processed_files', f'{sample_id}.xlsx')
            if not os.path.exists(input_excel):
                logging.error(f"Input Excel file not found: {input_excel}")
                return
                
            print(f"\n{'='*50}")
            print(f"Processing sample: {sample_id}")
            print(f"Reading content from: {scraped_file}")
            print(f"{'='*50}\n")
            
            # Read content from scraped file
            with open(scraped_file, 'r', encoding='utf-8') as f:
                content = f.read()
                
            if not content:
                logging.warning(f"No content found in scraped file for sample {sample_id}")
                return
                
            recommendations = self.parse_cpic_content(content)
            if recommendations:
                self.process_excel_file(input_excel, recommendations)
            else:
                logging.warning("No recommendations found in the content")
    ...
    def extract_gene_info(self, drug):
        ...
        try:
            ...
            content_dir = os.path.join('test_output', 'scraped_content')
...
def main():
    processor = TestSampleProcessor()
    
    # Get test sample ID
    test_sample = "N1DL5740"
    processor.current_sample = test_sample
    
    logging.info(f"Processing test sample: {test_sample}")
    print(f"\n{'='*50}")
    print(f"Processing sample: {test_sample}")
    
    # Find the content file for this sample
    content_dir = "test_output/scraped_content"
    content_files = [f for f in os.listdir(content_dir) if f.startswith(f"{test_sample}_processed_content")]
    
    if not content_files:
        print(f"No content file found for sample {test_sample}")
        return
        
    content_file = os.path.join(content_dir, content_files[0])  # Take the first matching file
    ...
    try:
        ...
        # Parse CPIC content
        recommendations = processor.parse_cpic_content(content)
        
        if recommendations:
            # Process Excel file
            excel_path = f"step1_phenotype_genotype_output/processed_files/{test_sample}.xlsx"
            processor.process_excel_file(excel_path, recommendations)
            
            # Record changes in aggregate file
            processor.record_changes_in_aggregate(test_sample, recommendations, True)
        else:
